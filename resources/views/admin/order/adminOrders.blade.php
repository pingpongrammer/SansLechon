...
<section class="grid grid-cols-1 ">
    <div style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <div class="flex mt-2 m-3 bg-[#e0865c] hover:bg-[#aa613f] p-2 rounded-lg duration-300 items-center text-orange-50 relative z-10" onclick="toggleDropdown()" id="dropdownButton">
        ...
        <table class="w-full border border-[#A65B37]">
            ...
            <tbody>
                ...
                <tr class="border border-[#A65B37] text-sm sm:max-2xl:text-base">
                    ...
                    @if($letchon->status == 'new')
                    <td class="p-3 whitespace-nowrap text-xs"><span class="p-1 bg-green-300">New Order</span></td>
                    @endif
                    @if($letchon->status == 'pending')
                    <td class="p-3 whitespace-nowrap text-xs"><span class="p-1 bg-yellow-300">Pending Order</span></td>
                    @endif
                    @if($letchon->status == 'cancel')
                    <td class="p-3 whitespace-nowrap text-xs"><span class="p-1 bg-red-300">Cancelled</span></td>
                    @endif
                    @if($letchon->status == 'success')
                    <td class="p-3 whitespace-nowrap text-xs"><span class="p-1 bg-blue-300">Success</span></td>
                    @endif
