...
<div class="col-lg-12 mt-5">
    ...
    @if($category->shop == 'Lechon')
        <div class="pb-4 overflow-auto">
            <div class="d-flex flex-column flex-md-row align-items-center justify-content-between bg-brown-body px-3 rounded overflow-auto">
                <img class="card-img img-fluid p-1 mr-3" src="/assets/img/Lechon.png" alt="Card image cap" id="product-detail" style="width:115px; height:115px">
                    ...
                    <div class="my-3 mr-3">
                        <h6 class="mb-0 order-text">{{$category->shop}} / {{$category->size}}</h6>
                    ...
                    <form method="POST" action="{{ url('cartDelete', $category->id) }}">
                        @csrf
                    <button type="submit" class="btn btn-danger btn-sm m-2"><i class="fa fa-trash mx-2"></i>Delete</button>
                    </form>
            </div>
        </div>
        @endif
            ...
            <div class="pt-1">
                <div class="d-flex flex-column flex-md-row align-items-center justify-content-between bg-brown-body px-3 rounded">
                    <img class="card-img rounded-0 img-fluid" id="preview" src="{{$category->categories->img ? asset ('storage/' .$category->categories->img) : asset('/storage/no/-image.png')}}" alt="" style="width:85px; height:85px"/>
                        ...
                        <h6 class="text-capitalize order-text m-1">{{$category->categories->type}},</h6>
                        <h6 class="text-capitalize order-text m-1">{{$category->categories->description}}</h6>
                        <h6 class="text-capitalize order-text m-1">({{$category->size}})</h6>
                        </div>
                        <h6 class="text-capitalize order-text">₱{{$category->price}}</h6>
